...
if (cluster.isMaster) {
  ...
  app.get('/x', async (req, res, next) => { // for test cacheing
    const id = Math.random() > 0.25 ? Math.floor(Math.random() * 100) : Math.floor(Math.random() * (10000000 - 100)) + 100;
    const [songInfo] = await queryDB(`SELECT * FROM songs WHERE id=${id}`);
    res.json(songInfo);
  });

  app.get('/xcache', async (req, res, next) => { // for test cacheing
  ...
  // how many bytes are already loaded
    ...
    fs.open(songInfo.filepath, 'r', (status, fd) => {
      ...
      fs.read(fd, buffer, 0, buffer.length, startingByte, (err, bufferSize) => {
        ...
        // console.log(`${startingByte}-${startingByte + (buffer.length)}/${songInfo.filesize}`)
